...
export function displayScanResult(scanResult: ScanResult): void {
	...
	if (scanResult.isMonorepo && scanResult.packages.length > 0) {
		console.log(
			`  └── packages/             (${scanResult.packages.length} packages detected)`,
		);
		...
		for (let i = 0; i < scanResult.packages.length; i++) {
			...
			console.log(
				`      ${prefix} ${pkg.relativePath.split("/").pop()}/`.padEnd(24) +
...
export async function selectPresets(
	...
	const languageOptions = DETECTION_RULES.filter(
		(r) => r.category === "languages",
	).map((r) => ({
		value: r.preset,
	...
	const toolOptions = DETECTION_RULES.filter((r) => r.category === "tools").map(
		(r) => ({
			value: r.preset,
			label: `${r.label}${detectedPresets.has(r.preset) ? " (detected)" : ""}`,
			hint: detectedPresets.has(r.preset) ? "recommended" : undefined,

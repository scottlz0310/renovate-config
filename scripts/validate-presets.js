...
async function validateFile(filePath) {
	...
	if (externalValidatorAvailable) {
		...
		if (!externalValidatorWarned) {
			console.warn(
				"renovate-config-validator not available; falling back to AJV schema validation.",
			);
			externalValidatorWarned = true;
		}
	}
	try {
		...
		if (schema === undefined) {
			...
			try {
				...
				if (!res.ok)
					throw new Error(
						`Failed to load schema ${schemaUrl}: ${res.statusText}`,
					);
				...
				if (!schemaFetchErrors.has(schemaUrl)) {
					console.warn(
						`Failed to fetch schema ${schemaUrl}: ${fetchErr}. Trying local/disk cache.`,
					);
		...
		if (!validate) {
			const ajv = new Ajv({
				...
				loadSchema: async (uri) => {
					...
					if (!r.ok)
						throw new Error(`Failed to load schema ${uri}: ${r.statusText}`);
					return r.json();
				},
			});
			addFormats(ajv);
		...
		if (schema === null) {
			console.warn(
				`Skipping full schema validation for ${filePath} due to missing schema; performing basic checks.`,
			);

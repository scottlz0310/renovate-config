...
- **Android preset** - New `languages/android` preset for general Android app development (Kotlin/Java, Gradle)
  - Groups AndroidX / Google Android ecosystem / Kotlin libraries and plugins
  - Includes Gradle wrapper update grouping
- **CLI detection** - Android detection added to `renovate-config-init` CLI
  - Detects `app/build.gradle(.kts)` to avoid generic JVM Gradle false positives
